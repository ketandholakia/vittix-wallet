...
| A7 | **User identity for collaboration** | **Step 1 done.** `Users` table (uuid, displayName, optional email, timestamps) + `UserDao` (watch/get/insert/findOrCreate/update/delete) and `userDaoProvider`. Users are global, not wallet-scoped, because one person can belong to several wallets. Schema v14. **Sized for step 2/3:** 243 `actorAccountId` call sites; wallet-membership identity is referenced across 12+ feature files plus sync_service.dart. `accountId` *is* the actor identity, so this is a rename of the actor across RBAC, the audit trail, invitations, splits, goals, allowances, bills and sync - to be done incrementally behind a bridge (nullable `userId` on WalletMembers + overloads), not in one commit. | L | in progress |
