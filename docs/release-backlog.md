...
| A6 | **Integer money + rounding policy** | ✅ **Done.** 17 money columns now store integer minor units (paise) via `integer().map(const MoneyConverter())` while Dart keeps using double major units, so no call sites changed. The two rate columns (`loans.interestRate`, `expense_split_members.percentage`) deliberately stay REAL. Monthly aggregation now runs as SQL SUM over exact integers and converts once at the boundary (`getMonthlySummaryTotals`, `getMonthlyExpensesForLastNMonths`) instead of accumulating floats. `allocateMinorUnits` / `allocateMinorUnitsByWeight` give splits a largest-remainder policy so they conserve value exactly. Schema v13. No data migration was written - the app is pre-production and dev installs are recreated. | L | - |
| A7 | **User identity for collaboration** | **Steps 1-2 (partial).** Step 1: `Users` table + `UserDao` + `userDaoProvider`; users are global, not wallet-scoped. Step 2 (bridge, additive): nullable `userId` FK on `WalletMembers` pointing at `Users`, so membership rows can carry a real identity while the old path keeps working. Schema v15. **Step 3 NOT started:** migrating the 243 `actorAccountId` call sites across RBAC, the audit trail (`wallet_activities.actorAccountId`), invitations, splits, goals, allowances, bills and sync to `userId`, then dropping `accountId`. This must be done feature by feature with the RBAC suite green at each step - not as one commit. | L | in progress |
...
- Users table + UserDao (A7 step 1) - with 7 tests.
- A7 bridge column: WalletMembers.userId (additive, no call sites migrated yet).
- Full suite 375 pass / 1 pre-existing failure.
