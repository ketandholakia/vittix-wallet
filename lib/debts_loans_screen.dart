...
final loansStreamProvider = StreamProvider<List<LoanDb>>((ref) {
  ...
  return db.select(db.loans).watch();
});

final peerDebtsStreamProvider = StreamProvider<List<PeerDebtDb>>((ref) {
  final db = ref.watch(databaseProvider);
  return (db.select(db.peerDebts)..orderBy([(pd) => OrderingTerm.desc(pd.date)])).watch();
});
...
class _LoansTab extends ConsumerWidget {
  ...
  Future<double> _getAccountBalance(AppDatabase db, int accountId) async {
              ...
              if (activeLoans.isEmpty) ...[
                ...
                ...activeLoans.map((loan) {
                  return FutureBuilder<double>(
                    future: _getAccountBalance(db, loan.accountId),
                ...
                ...inactiveLoans.map((loan) {
                  return Card(
                    ...
                    shape: RoundedRectangleBorder(
                      ...
                      side: BorderSide(color: Theme.of(context).dividerColor.withOpacity(0.04)),
                    ),
                    child: ListTile(
                    ...
                    child: ListTile(
                      leading: Container(
                        ...
                        decoration: BoxDecoration(
                          color: isLent ? Colors.teal.withOpacity(0.08) : Colors.deepOrange.withOpacity(0.08),
