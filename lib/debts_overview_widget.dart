...
import 'package:expense_tracker/app_database.dart' hide Column;
...
final debtsSummaryProvider = FutureProvider<Map<String, dynamic>>((ref) async {
  ...
  final peerDebtsFuture = (db.select(db.peerDebts)..where((pd) => pd.isSettled.equals(false))).get();
  final activeLoansFuture = (db.select(db.loans)..where((l) => l.isActive.equals(true))).get();
  ...
  final accounts = await (db.select(db.accounts)
        ..where((a) => a.id.isIn(activeLoans.map((loan) => loan.accountId).toList())))
      .get();
  final balancesByAccountId = await db.accountDao.getBalancesForAccounts(accounts);
...
class DebtsOverviewWidget extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final summaryAsync = ref.watch(debtsSummaryProvider);
